...
- **`dynamicActionTypes`** on the renderer — widen the carrier list: `dynamicActionTypes={[...DEFAULT_DYNAMIC_ACTION_TYPES, 'KanbanBoard']}`. The prop replaces the default, so spread it in. Custom (BYOC) types are not carriers until listed.
...
A gated dispatch is silent: nothing is thrown, the spec stays valid, no automatic confirm is raised, the control stays live and the fields feeding it do not freeze. `local` handlers and `false` denials are not gated — only the kinds that leave the renderer (`agent`, `host`, `recompose`).
