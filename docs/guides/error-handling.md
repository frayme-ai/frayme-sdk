...
| Class | Status | Codes |
| --- | --- | --- |
| `APIConnectionError` | None | Network failure, timeout, or a connection that died mid-stream |
| `APIUserAbortError` | None | You aborted (signal or `stream.abort()`) |
...
| `RateLimitError` | 429 | `RATE_LIMITED` (carries `retryAfter`, in seconds) |
...
Status 429 covers two very different situations, and a retry loop that conflates them will hammer the API pointlessly for the rest of the month:
