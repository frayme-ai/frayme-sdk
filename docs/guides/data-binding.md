...
- `{ "$bindState": "/path" }` — **two-way bind**: the control reads its value from the path and writes back on input. Typing never leaves the browser.
- `{ "$state": "/path" }` — **read**: resolves the path's live value into a prop or an action param at render / fire time.
...
This is why `data` matters beyond correctness of copy: facts you pass in `data` are seeded into `state`, so repeats driven by them actually render. Frayme's validators flag an unseeded `repeat.statePath` for exactly this reason — and note that seeding `[]` passes the check but *still renders nothing*; use an empty seed only when your host genuinely fills the list at runtime (for example via `initialState` on the renderer).
