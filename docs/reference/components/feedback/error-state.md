# ErrorState

Failure panel (role="alert"): danger-toned icon + title + detail, with an optional retry action (children, e.g. a Button). Use when a load/action failed.

Accepts child elements via `children` (the `default` slot).

## Example

```json
{
  "root": "error-state",
  "elements": {
    "error-state": {
      "type": "ErrorState",
      "props": {
        "title": "Something went wrong",
        "detail": "We could not load your data. Please try again."
      }
    }
  }
}
```

## Props

| Prop | Type | Description |
| --- | --- | --- |
| `title` | `string` | The failure headline (e.g. "Something went wrong"). Short and neutral — pair with `detail` for the specific error/guidance. |
| `detail` | `string` | Secondary explanation of what went wrong (e.g. an error message or guidance). |
| `icon` | `string` | Danger-toned icon glyph by NAME from the closed icon registry (or a single emoji glyph, rendered as-is) (default "alert-triangle"). Never raw SVG; unknown names render nothing. |
| `size` | `"sm" \| "md" \| "lg"` | Icon + type scale and vertical padding (default md). |
| `align` | `"center" \| "start"` | Content alignment: center (default) · start (left-aligned). |
| `mutedColor` | `string` | Secondary/muted text colour — the detail explanation line under the title (default the muted-foreground token). The icon stays danger-toned. |
| `fontSize` | `string \| number` | Exact title font-size (e.g. 22px / 1.375rem). Overrides ONLY the title type scale of the `size` enum (the default); icon size + panel padding stay on `size`. |
| `weight` | `"light" \| "normal" \| "medium" \| "semibold" \| "bold"` | Title font weight (default semibold). Reach for `bold` for a heavier heading or `medium`/`normal` for a softer one. |
| `font` | `"sans" \| "serif" \| "mono" \| "rounded" \| "display"` | Typeface for the whole error panel; cascades to descendants via font inheritance (sans · serif · mono · rounded · display). Omit to inherit the theme font. |
| `tracking` | `"tighter" \| "tight" \| "normal" \| "wide" \| "wider"` | Letter-spacing of the `title` (tighter · tight · normal · wide · wider; default normal). |
| `leading` | `"tight" \| "snug" \| "normal" \| "relaxed" \| "loose"` | Line-height of the `title` (tight · snug · normal · relaxed · loose; default normal — sm sets it explicitly, md/lg inherit the browser normal). |
| `titleLevel` | `"h1" \| "h2" \| "h3" \| "h4"` | Heading level for the `title` (default h3 — a state panel is content inside a Card, whose title is h2). Set it to h2 when the panel sits directly under the PageHeader with no Card around it, so the screen's headings nest legally — a document that jumps h1 to h3 has no outline for anyone navigating by heading. |
