...
| [PermissionMatrix](permission-matrix.md) | A role × capability permission grid: each cell is a tri-state toggle (allow / deny / inherit) that cycles on c |
| [PrintLayout](print-layout.md) | A print-ready paginated document: author-supplied sections (heading, paragraph, key-value fields, table, image |
| [Scheduler](scheduler.md) | A professional day/week scheduler: a vertical hour axis with one column per day or resource, events as duratio |
| [SignaturePad](signature-pad.md) | A signature pad: draw a signature with mouse/touch/stylus and it renders as smooth SVG polylines, with an opti |
| [Stopwatch](stopwatch.md) | A stopwatch that counts up from zero, with a mm:ss / hh:mm:ss readout (optional centisecond precision), Start  |
| [Timer](timer.md) | A countdown timer that ticks down from `duration` seconds to zero, with a mm:ss / hh:mm:ss readout, optional S |
| [TournamentBracket](tournament-bracket.md) | A sport-agnostic tournament card with a group-stage standings view AND a knockout bracket. The standings colum |
