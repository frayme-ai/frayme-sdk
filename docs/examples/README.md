...
- **Local interactivity by default.** Roughly 90% of what you can do in these demos, from search filters and tab switches to seat selection and slider drags, resolves inside the renderer through state bindings (`{"$bindState": "/path"}`). No network round-trip, no agent involvement.
...
- **Everything is a validated json-render spec.** Each demo is a stream of operations like `{"op":"add","path":"/elements/board","value":{...}}`. That is the same wire format `POST /v1/compose` streams, validated against the 189-component catalog before it renders.
