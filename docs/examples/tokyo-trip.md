...
The `DateRangePicker` binds into state, so the reservation label tracks the user's date edits locally — no round-trip until they commit.
...
const stream = frayme.compose.stream({
  ...
  data: { trip: {/* flights, hotel, day plans — rendered verbatim */} },
  actions: [
    {
      name: 'reservePlan',
