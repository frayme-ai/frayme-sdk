...
- [ ] **Requests fit the size ceilings.** The API answers 400 to `data` or `prior_spec` over 48,000 characters of JSON, or `action_context` over 16,000, and sending the same request again fails the same way. `fraymeTools` and `FraymeScreen`'s `continue` cut presses and pressed screens to fit for you, and `createActionTool` cuts presses, but `data` is never cut: keep it under 48,000 characters, or page it.
