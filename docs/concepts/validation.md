...
1. The failing attempt is discarded. If you were streaming, you receive `compose.restarted` — discard rendered state (the SDK does this for you; see [Streaming](streaming.md)).
...
{% hint style="info" %}
This is why there is no "sometimes the AI returns broken JSON" handling anywhere in a Frayme integration. The failure mode is an explicit, typed error — never a malformed spec.
{% endhint %}
