...
Because `params` can carry `$state` references and `state` is a live snapshot, the user's actual input arrives with the event — no form-scraping glue code.
...
| `recompose` | Frayme regenerates the UI itself using a fixed `prompt` you supply on the action — a self-contained "next screen" with no agent round-trip. |
