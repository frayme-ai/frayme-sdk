...
| Agent transport | [AG-UI](https://github.com/ag-ui-protocol) (Linux Foundation) | *How* agents talk to UI | `@frayme/runtime/ag-ui` delivers specs as `frayme:spec` events and actions as the `frayme:action` frontend tool. |
...
- **Not a code generator.** Frayme never emits HTML, React source, or arbitrary JavaScript — only declarative JSON specs, validated against a closed component catalog. That is what makes the output safe to render.
