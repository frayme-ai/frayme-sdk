...
| Retry after a **success** | **Free replay** of the stored result, carrying the original `generation_id` and `replayed: true`. Nothing is billed. |
...
- **`stream: true`** gives an instant burst-stream: `compose.started`, every stored `op` back-to-back, then `compose.completed` with `"replayed": true`. Your rendering path doesn't need a special case.
- **`stream: false`** gives the stored envelope with `"replayed": true`.
