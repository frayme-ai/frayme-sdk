...
The browser never holds your key. Instead, a route in your app checks the caller, adds the key and forwards compose calls to Frayme. `createFraymeHandler` is that route, ready-made:
...
export const POST = createFraymeHandler({
  // Allows local development only. Replace it with your own session check
  // before you deploy: this route spends your quota.
  authorize: () => process.env.NODE_ENV === 'development',
});
```

This is [keyless proxy mode](authentication.md#keyless-proxy-mode): the `@frayme/api` client in the browser points at `/api/frayme`, and only this route knows the key. The handler reads `FRAYME_API_KEY`, accepts only compose fields, streams the answer straight through, and keeps only the actions a request declares, which is why the page below declares its one action. See [Server handler](../guides/server-handler.md) for every option.
      ...
      <button
        ...
        onClick={() =>
          compose({
            ...
            // The server route only keeps declared actions, so declare this one.
            actions: [{ name: 'create_account', role: 'Create account', required: true }],
          })
        }
      >
